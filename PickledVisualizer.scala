...
import reflect.internal.pickling.{PickleBuffer, ByteCodecs}
import reflect.internal.pickling.PickleFormat._
...
trait A {
  // Use of 't as an annotation argument adds a member
  // `name` to the enclosing trait, visible only in
  // separate compilation.
  @strangeTest('t) def test(x: String): Unit
}
